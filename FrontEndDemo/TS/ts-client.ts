...
// but it lets use see TS classes
...
export class ApiClient {
    ...
    // We can define "parameters properties" this lets us do
    // access mofifier (public/private/protected), readonly (or not)
    // name, and type ("and default value")
    constructor(private readonly baseUrl: string = "http:localhost:5190") { }

    // Methos would go down here - methods in classes
    // can be static or instance methods/properties - same logic as C#
    
    // TS has generics - we represent a generic with the 'T' - same as C#
    // just means it can work any type
    // This method return wither an object of some type OR an ApiError (we define that)
    async getJson<T>(path: string): Promise<T | ApiError>{
        ...
        try{
            ...
            // _client.hetJson<InventoryItem>() - this is how i'd call this
