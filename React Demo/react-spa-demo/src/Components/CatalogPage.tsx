...
// CatalogPage will be composed of many BokkCards - they will be child components of
...
export function CatalogPage() {
    ...
    // Hook are functions that allow you to do certain things based on the component lifecycle
    ...
    // triggering an immediate re-render. Subsequent calls to setState() for a giveb state value
    // will trigger re-renders as normal.
    useEffect(() => {
        // Flag to allow for chopsing the correct fState value later
        let active = true;
    ...
    const visibleBooks = [...items]
        .filter((i) => i.name.toLocaleLowerCase().includes(userQuery.toLowerCase()))
        .sort((a, b) => 
    ...
    if (fState === "failed" ) return <p>Could not reach the API. Is it running on :5173? Check CORS.</p>
    ...
    //     <>
    //     <div className="toolbar">
    //         <h2>Catalog</h2>    {/* When oir button is cliecked, invert the value of compact */}
    ...
    //         {
    //             items.map((item) => (
    //                 // The minimun to render a bookcard is a item and compact - its props
