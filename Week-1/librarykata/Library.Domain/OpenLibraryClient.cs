...
{
    // We are going to create ONE HTTPClient for the entire process.
    // If you use one per call, you are going to leak sockets - and eventually trigger a SocketException
    private static readonly HttpClient client = new();

    // We are going to write an async method is ANY method that calls async code.
    ...
    // A task in C# is like a Promise in JS - its a placeholder in memory telling the runtime
    // I expect there to be a LibraryItem (or whatever the Task is "wrapping" with it's brackets)
    // when this methods resolvs. I have no idea when that is, so far now - hold that plece with a Task
    // We are also going account for the possibility of a null - because my HTTP call could fail for a number reasons
    public async Task<LibraryItem?> FetchByIsbnAsync(string isbn)
    {
        // I'm going to create a string to hold the urlI'm targeting
        ...
        // This could fail for a ton of reasons - Idon't control OpenLibrary OR the Internet between my laptop
        // and their servers
        try
        {
            // We are going try to get back a json formatted string from API
            string jsonResponse = await client.GetStringAsync(url);

            // We are going to write our own parsong logic in a method called Parse()
            // Thankfuly the returns from OpenLibrary are small. For an unmanageable return
            // se PokeAPI
        ...
        {
            Log.Warning("FetchByIsnAsyn failed: {Message}", ex.Message);
            return null;
        }
    }

    // We are going to write our own parsong logic
    // Mostly as an exercise to work with Json
    public static LibraryItem? Parse(string json)
    {
        ...
        // is a "docs" array. If we find the book we want based in it's isbn that searched for, it's inside that array
        ...
        // if the docs array has length of 0, then we got the correct object shape back, but our isbn didn't match anything - no book to return
        ...
        JsonElement foundBook = docs[0]; // If we get something back, we should only get one thing. We seached by isnb - they're unique

        // Now we can unpack things about this foundBook
        // We are using the ?? null coallescing operator.
        // IF somwthing is there: return the value resulting from the code to the left of the ??
        ...
        // Getting the author is less straightfoward because of the fact that books can have more than one author
        // So there is not a single value author property, it's another array
        string author = "Uknown";

        // Checking to see if we have the author array, and if it's there grab the firs author
