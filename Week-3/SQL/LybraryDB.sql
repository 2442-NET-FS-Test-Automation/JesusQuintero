/*
This script is intended to be crearly re-runnable. I should be able to
run it top to bottom and recreate my database in place - this is for demo purposes.

I'm going to break this script into sections, denoted by commets
*/

-- At the top of my file, before any

USE LibraryDB;
GO -- GO is MS SQL Server specific
...
-- TRUNCATE - Deletes all data in a table - preserves structure (colums + constraints)
-- ALTER - Used to edit the structure of an existing table (add columns, tweak constraintes, etc)
-- ==============================================
-- I will come back once we've created out tables, and included a DROP section
...
-- associates it with this default dbo schema. So the full table names becomes:
-- dbo.Author
CREATE TABLE dbo.Author 
(
    -- Colum-name data-type constrains (optional)
    ...
    -- Sfter I define my colums, datatypes and basic constraints
    -- I can optionally add some named constraits. If I don't name constraints,
    -- nothing breaks BUT I can make my life easier and make error message more
    -- functional/readable by explicitly namyng my constraints.
...
(
    -- This is perhaps daster but not best practice. For example, lest say I want to change from MemberID as my PK
    -- to Email as my PK. By doing an in-line non-named constraint, I've shot myself in the foot . It's much more harder
...
(
    ...
    AvailableCopies INT NOT NULL CONSTRAINT DF_Book_ATotalCopies DEFAULT(1)
    ...
    -- When I set a FK  constraint, I can optionally set the delete behavoir via ON DELETE
    -- CASCADE - VERY RISKY - If an author is deleted, all their books go too.
    -- SET NULL - SAFE - If an author is deleted, this AuthorId in Book is set to null. (requires a nullable column)
    -- NO ACTION - SAFE - Same as restrict in MS SQL Server
    -- SET DEFAULT - Low risk - Requires a default value constraint, will set the FK column to that value if author is deleted
    ...
    -- Available copies CANNOT exeed TotalCopies
    CONSTRAINT CK_Book_Copies CHECK (AvailableCopies <= TotalCopies)
);
...
-- Now that I've create the tables, using CREATE (DDL), how can I edit the tables themselves?
...
-- stuck ALER-ing it. Prior to giving a table any data, it is often easier to drop the table and
...
-- SELECT - Used to select a record or records. (This is where ptrer SQL
