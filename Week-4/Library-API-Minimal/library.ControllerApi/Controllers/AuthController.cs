...
public class AuthController : ControllerBase
{
    ...
    // POST /auth/regiister {username, password} -> 201
    // ONLY creates consumer accounts, admins should be seeded
    // do no make a register as admin endpoint visible to users
    [HttpPost("register")]
    public async Task<ActionResult> Register(RegisterDto dto)
    {
        var error = await _users.RegisterAsync(dto.Username, dto.Password);

        if( error is not null) return Conflict(new {error}); // 409 Conflict - duplicate username

        return CreatedAtAction(nameof(Me), null); // 201 - this would make use of another endpint for its return
    }
    ...
    public async Task<ActionResult> Login(LoginDto dto)
    {
        ...
        if(user is null) return Unauthorized(new {error = "Bad credentials"}); // 401 - same for bad pass/username
